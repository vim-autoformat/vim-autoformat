...
if showdiff:
    ...
    elif int(vim.eval('exists("g:autoformat_diffcmd")')):
        diffcmd = vim.eval('g:autoformat_diffcmd')
...
if stderrdata:
    ...
    if showdiff:
        p = subprocess.Popen(diffcmd + " " + vim.current.buffer.name + " -", env=env, shell=True, stdin=PIPE, stdout=PIPE, stderr=PIPE)
        stdoutdata, stderrdata = p.communicate(stdoutdata)
        vim.command("botright new | setlocal buftype=nofile ft=diff bufhidden=wipe nobuflisted noswapfile nowrap")
        stdoutdata = ("--- Automatically generated by vim-autoformat due to g:autoformat_showdiff"
            + os.linesep + "--- You can see what difference Autoformat would do."
            + os.linesep + "--- ***"
            + os.linesep + stdoutdata)
